	...
	<div class="col-lg-12 col-12 p-4" id="main_content">
		<div class="card shadow mb-4">
			<div class="card-header py-3">
				<h6 class="m-0 font-weight-bold text-primary">Izmijeni knjigu</h6>
			</div>
			<div class="card-body">
				<form id="update-form">
					...
					<div class="inputWrapper  m-2 ">
						<button class="btn btn-success" id="change-book" type="button" data-save-book>Sačuvaj</button>
					</div>
				</form>
			</div>
		</div>
...
<script>
	document.addEventListener('DOMContentLoaded', () => {
		...
		getBook({{$id}}).then(response => {
			...
			$("#publisher").val(response.book.publisher);
			$("#printer").val(response.book.print);
			$("#status option").each(function(){
				if($(this).val() == response.book.status){
					$(this).attr("selected","selected");
				}
			});
			$("#category_id option").each(function(){
				if($(this).val() == response.book.category_id){
					$(this).attr("selected","selected");
				}
			});
		});
		...
		// updating book logic
		document.querySelector('#change-book').addEventListener('click', (e) => {
			e.preventDefault()
			let name = document.querySelector('#title').value;
			if (name === "") {
				alert('Popunite potrebne podatke!');
			} else {
				updateBook($("#update-form")[0]);
			}
		});
